...
(`maxfps`/vsync pacing is disabled for the duration of the run) and times every main-loop
iteration with the OS performance counter, not the engine's own integer-millisecond clock. It
...
you can see it if you want to but it's not part of any "last N" summary. The window stays open
(`pause`) at the end so it doesn't vanish before you can read the results.
...
results in `benchresults.csv` next to the script, its own throwaway `bench-home-run/` profile
...
`benchframes-<row>.csv` next to `benchresults.csv` - one raw frame time (ms) per line, in
capture order, for whichever row number that pass was. Set `benchdumpframes 0` in a profile to
skip that if you don't need the raw samples.

### Formulas (verbatim from the engine source, `patches/22-benchmark.patch`)
...
```
worstcount = max(1, floor(n/100))
worst_mean_ms = mean of the worstcount largest frame times
onepctlow_fps = 1000.0 / worst_mean_ms
```

**Stutters**: count of frames whose time is more than 2x the median (`p50`):

```
stutters = count of frame_ms[i] such that frame_ms[i] > 2.0 * p50
```
