...
- 👁️ **Flat high-visibility players.** Everyone renders as a solid team colour — **teammate blue, friend cyan, enemy red, friend-on-enemy orange** — so you read fights instantly. Purely visual and client-side.
...
- 📊 **Expanded scoreboard.** Frags in every mode (including CTF), deaths, KDR, captures, optional accuracy/damage — all toggleable.
...
- ⏱️ **Reload feedback, your way.** A 3-2-1 rifle-reload **countdown** (9 styles incl. *Collapse*, *Edge-beats*) and an independent fire-**ready flash** (8 styles from a subtle *Thin-ring* to a bold *Edge-pulse*), plus an optional tick-tick-tick-**ding metronome**. Mix and match.
...
- ⚙️ **Native settings.** All SwiftGibs options live in the regular **Esc → options** menu as **SwiftGibs** and **Friends** tabs. **F3** is a quick-play launcher for instant bot matches.
- 🖥️ **Just works.** Auto-detects your monitor's native resolution and launches fullscreen on first run. No config needed.
- 🌐 **Server-compatible.** No protocol changes — every feature is client-side. Play on any official/public Sauerbraten server.
...
### macOS (Apple Silicon)
1. Download `SwiftGibs-mac.zip` and unzip it → `SwiftGibs.app`.
2. **First launch only:** right-click (or Control-click) `SwiftGibs.app` → **Open** → **Open** again.
   macOS blocks unsigned apps on a normal double-click; this one-time step tells it you trust it. After that, double-click works.
...
- **Move / shoot** like any Sauerbraten client. It's instagib: you spawn with the rifle, one hit kills.
- **F3** — Quick Play: launch an instant bot match (FFA / Team / CTF), add bots, or jump into settings.
- **Esc → options** — the **SwiftGibs** tab (server filter, scoreboard, highlighting, reload countdown/flash/sound, teammate crosshair) and the **Friends** tab (add/remove, see who's in-game).
- **Server browser** — filtered to instagib-family servers by default (toggle in the SwiftGibs tab).
...
| **Metronome** | audio tick·tick·tick·ding over the reload | Off *(default)* · Flat · Rising · + volume |
...
- ✅ Flat team colours, a clean HUD, and the **teammate** crosshair are all clarity/safety features (the teammate indicator ships in stock Sauerbraten).
- ❌ We deliberately **do not** add an enemy-target crosshair, aim assist, wall-hacks, or anything that gives an offensive advantage over players on a stock client.

It stays a legit competitive config you can take onto public servers.
...
- **`patches/NN-*.patch`** — the engine changes, applied in order. Each is one focused feature (server filter, scoreboard, friends, reload feedback, flat models, …).
- **`overlay/`** — menus, autoexec, crosshair, flat skins: pure data layered over the stock game.
- **`build/integrate-menus.sh`** — splices the SwiftGibs/Friends tabs into the stock options menu at bundle time.
- **`tools/strip-assets.sh`** — produces the flat low-res data tree from a Sauerbraten install (or the official release archive).

Releases are built reproducibly in GitHub Actions from the official 2020 Sauerbraten data — no local install required.
...
- **Engine code:** zlib license (see `vendor/sauer2020/`). SwiftGibs' modifications are provided under the same license and are plainly marked as an altered version.
...
SwiftGibs is a fan-made competitive config, not affiliated with or endorsed by the Sauerbraten project. Play nice, gib fast. 🔫
